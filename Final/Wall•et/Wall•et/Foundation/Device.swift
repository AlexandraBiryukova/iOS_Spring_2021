...
private enum Constants {
    static let allSmallPhones: [Device] = [.iPhone5, .iPhone5c, .iPhone5s, .iPhoneSE]
}
...
enum Device {
    ...
    var isSmall: Bool {
        ...
        case let .simulator(device):
            return device.isOneOf(Constants.allSmallPhones)
        case .unknown:
            return false
        default:
            return isOneOf(Constants.allSmallPhones)
        }
    }
...
extension Device {
    static let current = Device.mapToDevice(identifier: Device.identifier)
